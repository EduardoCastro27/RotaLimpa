...
class _RouteMapScreenState extends State<RouteMapScreen> {
  ...
  bool _calculandoRotaReal = false;
  ...
  Future<void> _carregarLocalizacaoInicial() async {
    try {
      ...
      // ========================================================
      // ROTA COM PONTOS FIXOS
      // ========================================================

      if (widget.rotaInicialModel?.possuiPontosFixos == true) {
        final pontosFixos = widget.rotaInicialModel!.pontos;
        ...
        // ======================================================
        // TENTA CALCULAR O TRAJETO PELAS RUAS REAIS
        // ======================================================

        if (pontosFixos.length >= 2) {
          setState(() {
            _calculandoRotaReal = true;
          });

          try {
            final rotaReal = await _roadRouteService.calcularRotaPelasRuas(
              pontosFixos,
            );

            if (rotaReal.length >= 2) {
              rota = rotaReal;
            }
          } catch (error) {
            debugPrint(
              'Erro ao calcular rota pelas ruas: $error',
            );

            // ==================================================
            // FALLBACK
            //
            // Se a Routes API não responder, continuamos usando
            // os pontos fixos da rota.
            // ==================================================
          }

          if (mounted) {
            setState(() {
              _calculandoRotaReal = false;
            });
          }
        }
      } else {
        // ========================================================
        // ROTA DE TESTE DOS OUTROS BAIRROS
        // ========================================================

        rota = _gerarRotaTestePorBairro(
          localAtual,
          _bairro,
        );
      }
      ...
      final rotaDetalhada = _detalharRota(
        rota,
        _distanciaEntrePontosMetros,
      );

      if (!mounted) return;

      final posicaoInicial =
          rotaDetalhada.isNotEmpty ? rotaDetalhada.first : localAtual;
      ...
      setState(() {
        ...
        _pontosConcluidos = List.filled(
          rotaDetalhada.length,
          false,
        );
  ...
  List<LatLng> _gerarRotaTestePorBairro(
    LatLng base,
    String bairro,
  ) {
  ...
  List<LatLng> _detalharRota(
    List<LatLng> rota,
    double intervaloMetros,
  ) {
    ...
    for (var i = 0; i < rota.length - 1; i++) {
      ...
      final passos = math.max(
        1,
        (distancia / intervaloMetros).ceil(),
      );
  ...
  void _atualizarLocalizacao(
    LatLng novaPosicao, {
    bool moverCamera = true,
  }) {
    if (!mounted) return;
  ...
  void _validarPontoNaRota(
    LatLng pontoAtual,
  ) {
    if (_rotaDetalhada.isEmpty || _pontosConcluidos.isEmpty) {
      return;
    }

    final proximo = _encontrarIndiceMaisProximoAdiante(
      pontoAtual,
    );
  ...
  MapEntry<int, double> _encontrarIndiceMaisProximoAdiante(
    LatLng atual,
  ) {
    var indice = math.max(
      0,
      _ultimoIndiceValidado,
    );
  ...
  double _calcularProgresso() {
    if (_pontosConcluidos.isEmpty) {
      return 0;
    }

    return _pontosConcluidos
            .where(
              (concluido) => concluido,
            )
            .length /
  ...
  void _simularPercurso() {
    ...
    _simulationTimer = Timer.periodic(
      ...
      (timer) {
        ...
        if (indice >= _rotaDetalhada.length) {
          setState(() {
            for (var i = 0; i < _pontosConcluidos.length; i++) {
              _pontosConcluidos[i] = true;
            }

            _ultimoIndiceValidado = _rotaDetalhada.length - 1;
        ...
        _atualizarLocalizacao(
          _rotaDetalhada[indice],
          moverCamera: indice % 6 == 0,
        );
  ...
  List<LatLng> get _rotaValidada {
    if (_ultimoIndiceValidado < 1) {
      return const [];
    }

    return _rotaDetalhada
        .take(
          _ultimoIndiceValidado + 1,
        )
        .toList();
  ...
  Future<void> _finalizarRota() async {
    ...
    try {
      ...
      final validados = _pontosConcluidos
          .where(
            (ponto) => ponto,
          )
          .length;
      ...
      final historico = RouteHistoryModel(
        ...
        distanciaPercorrida: _formatarDistancia(
          _distanciaPercorridaMetros,
        ),
      ...
      await _historyService.salvarHistorico(
        historico,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Não foi possível finalizar a rota: $error',
            ),
          ),
        );
      }
          ...
          child: Column(
            children: [
              ...
              ListTile(
                ...
                onTap: () => _abrirTelaDoMenu(
                  (_) => WeeklyRoutesScreen(),
                ),
              ),
              ListTile(
                leading: const Icon(
  ...
  String get _bairro =>
      widget.rotaInicialModel?.bairro ??
      widget.rotaInicial?['bairro'] ??
      'Centro';

  String get _tempo =>
      widget.rotaInicialModel?.tempoEstimado ??
      widget.rotaInicial?['tempo'] ??
      '2h30min';

  String get _motorista =>
      widget.rotaInicialModel?.motorista ??
      widget.rotaInicial?['motorista'] ??
      'Motorista';

  String get _veiculo =>
      widget.rotaInicialModel?.veiculo ??
      widget.rotaInicial?['veiculo'] ??
      'Caminhão';

  String get _turno =>
      widget.rotaInicialModel?.turno ?? widget.rotaInicial?['turno'] ?? 'Turno';
  ...
  String get _tempoReal {
    ...
    final duracao = (_fimRota ?? DateTime.now()).difference(
      _inicioRota!,
    );
      ...
      return Scaffold(
        body: Center(
          child: Padding(
            ...
            child: Text(
              _erro ?? 'Não foi possível obter a localização.',
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _foraDaRota ? Colors.orange : AppColors.primary,
                ),
              ),
              child: Row(
                children: [
            ...
            child: Column(
              children: [
                _BotaoMapa(
                  ...
                  onTap: _centralizarNoVeiculo,
                ),
                const SizedBox(
                  height: 12,
                ),
                _BotaoMapa(
                  icon: _rastreando ? Icons.pause : Icons.play_arrow,
                  onTap: _iniciarOuPararRastreamento,
                ),
                const SizedBox(
                  height: 12,
                ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Tempo base: $_tempo  •  '
                    'Validados: '
                    '$validados/${_pontosConcluidos.length}',
                  ),
                  const SizedBox(
                    height: 14,
                  ),
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        _finalizando ? 'Finalizando...' : 'Finalizar rota',
                      ),
                    ),
                  ),
                ],
